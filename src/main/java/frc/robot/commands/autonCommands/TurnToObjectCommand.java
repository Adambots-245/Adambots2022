...
public class TurnToObjectCommand extends CommandBase {
  ...
  public TurnToObjectCommand(DriveTrainSubsystem inpuDriveTrain, double inputSpeed,
      double cameraAngle, boolean resetGyro) {
  ...
  TurnToObjectCommand(DriveTrainSubsystem inpuDriveTrain, double inputSpeed,
      double cameraAngle) {
    this(inpuDriveTrain, inputSpeed, cameraAngle, true);
  }
  ...
  @Override
  public void execute() {
    double turnSpeed = gyroPIDSubsystem.getController().calculate(gyroPIDSubsystem.getMeasurement(), cameraAngle);
